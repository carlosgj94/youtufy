...
db = con.Youtufy
youtufy = db.playlist
confi = db.configuration
conf= confi.find({"configuration":"yes"})
configuration="no"
for key in conf:
	configuration=key["configuration"]
	music_name=key["music_name"]
	music_quality=key["music_quality"]
	if music_quality=="yes":
		music_quality= "-f bestaudio"
	else:
		music_quality=""
...
if configuration=="yes":
	while option!="0":
		...
		if option=="1":
			...
			os.system("cd && cd "+music_name+" && mkdir "+list_name+"&& cd "+list_name+" && youtube-dl --ignore-errors -o '%(title)s.%(ext)s'  --extract-audio --audio-format mp3 --audio-quality 0 "+url)		
			...
			if multiple =="1":
				for key in y_update:
					os.system("cd && cd "+music_name+" && cd "+key['name']+" && youtube-dl "+music_quality+"--ignore-errors -o '%(title)s.%(ext)s'  --extract-audio --audio-format mp3 --audio-quality 0 "+key['url'])
			elif multiple=="2":
				y_all = youtufy.find()
				i=0
				urls=[]
				...
				if selection>"0":
					os.system("cd && cd "+music_name+" && cd "+names[int(selection)-1]+" && youtube-dl "+music_quality+" --ignore-errors -o '%(title)s.%(ext)s'  --extract-audio --audio-format mp3 --audio-quality 0 "+urls[int(selection)-1])
			else:
			...
			if selection>"0":
				...
				local=input("y/n --> ")
				if local=="y":
					os.system("cd && cd "+music_name+" && rm -r "+keys[int(selection)-1])
					print("List erased")
			...
			if yesno=="y":
				...
				y_all2= y_all
				for key in y_all:
					files=os.listdir("/home/"+username+"/"+music_name+"/"+key["name"])
					for key2 in y_all2:
						if key["name"]!=key2["name"]:
							files2=os.listdir("/home/"+username+"/"+music_name+"/"+key2["name"])
							for file1 in files:
								for file2 in files2:
									if file1==file2:
										os.system("cd && cd "+music_name+"/"+key["name"]+" && rm -r "+re.escape(file1))

		elif option=="6":
			print("This song will be added to 'RandomSongs' directory")
			url = input("Enter the new url: ")
			os.system("cd && cd "+music_name+" && cd RandomSongs && youtube-dl --ignore-errors -o '%(title)s.%(ext)s'  --extract-audio --audio-format mp3 --audio-quality 0 "+url)
else: 
	print("You have to run first the conf.py file. Thanks :)")
